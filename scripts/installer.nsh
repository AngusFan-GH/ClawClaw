...
!macro customCheckAppRunning
  ; Remove stale machine-wide entries left by older/broken installers. ClawClaw
  ; is now intentionally per-user; keeping an old HKLM uninstall entry makes
  ; Windows "Apps & features" launch the wrong uninstaller path.
  ...
  ${if} ${isUpdated}
    ...
    nsExec::ExecToStack /TIMEOUT=20000 '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\run-gateway-cmd.ps1" -InstallDir "$INSTDIR" -Command "gateway stop"'
    Pop $0
    Pop $1
    nsExec::ExecToStack /TIMEOUT=20000 '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\run-gateway-cmd.ps1" -InstallDir "$INSTDIR" -Command "gateway uninstall"'
    Pop $0
    Pop $1
    nsExec::ExecToStack /TIMEOUT=20000 '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\kill-install-dir-processes.ps1" -InstallDir "$INSTDIR"'
    Pop $0
    Pop $1

    ; Release NSIS's current directory before moving the old install tree.
    SetOutPath "$TEMP"

    IfFileExists "$INSTDIR\" 0 _cc_old_install_dir_ready
      StrCpy $R8 0
    _cc_find_stale_dir:
      IfFileExists "$INSTDIR._stale_$R8\" 0 _cc_found_stale_dir
      IntOp $R8 $R8 + 1
      Goto _cc_find_stale_dir

    _cc_found_stale_dir:
      ClearErrors
      Rename "$INSTDIR" "$INSTDIR._stale_$R8"
      IfErrors 0 _cc_old_install_dir_moved
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$INSTDIR"'
        Pop $0
        Pop $1
    _cc_old_install_dir_moved:
      CreateDirectory "$INSTDIR"
    _cc_old_install_dir_ready:

    ; The old electron-builder uninstaller can mis-detect lingering child
    ; processes and block upgrades. Remove the old command entries so the
    ; built-in uninstallOldVersion step returns immediately; the new install
    ; writes fresh uninstall registry values later in the same section.
...
!macro customInstall
  ...
  ; Delete renamed old installation trees after the new app has been laid down.
  ; This is detached so first launch is not blocked by slow antivirus scans.
  IfFileExists "$INSTDIR._stale_0\" 0 _ci_stale_cleanup_done
    ExecShell "" "cmd.exe" `/c ping -n 31 127.0.0.1 >nul & cd /d "$INSTDIR\.." & for /d %D in ("$INSTDIR._stale_*") do rd /s /q "%D"` SW_HIDE
  _ci_stale_cleanup_done:
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\卸载 ${PRODUCT_NAME}.lnk" "$INSTDIR\${UNINSTALL_FILENAME}" "/currentuser"
