...
# Post-installation script for ClawClaw on Linux
...
# Create symbolic link for ClawClaw app binary
if [ -x /opt/ClawClaw/clawclaw ]; then
    ln -sf /opt/ClawClaw/clawclaw /usr/local/bin/clawclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ClawClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ClawClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/ClawClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ClawClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/clawclaw'
...
echo "ClawClaw has been installed successfully."
