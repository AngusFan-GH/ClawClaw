...
ClawClaw is a cross-platform **Tauri desktop app** (React 19 + Vite + TypeScript + Rust) providing a GUI for the OpenClaw AI agent runtime. It uses pnpm as its package manager (pinned version in `package.json`'s `packageManager` field).
...
- **Tauri on headless Linux**: A graphical display is required for window startup; use `$DISPLAY` (for example `:1` via Xvfb/VNC) in headless environments.
...
- **No database**: The app uses JSON settings files and the OS keychain. No database setup is needed.
...
- **Renderer/Main API boundary (important)**:
  ...
  - Do not add direct Tauri `invoke(...)` calls in pages/components; expose them through host-api/api-client instead.
