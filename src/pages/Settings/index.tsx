...
export function Settings() {
  ...
  const initialReminderItems = useMemo<ReminderItem[]>(
    () => [
      {
        id: 'reminder-privacy',
        text: t('reminders.defaults.privacy'),
        enabled: true,
      },
      {
        id: 'reminder-mail',
        text: t('reminders.defaults.mail'),
        enabled: true,
      },
      {
        id: 'reminder-delete',
        text: t('reminders.defaults.delete'),
        enabled: true,
      },
    ],
    [t]
  ...
  useEffect(() => {
    if (!initialized || reminders.length > 0) return;
    setReminders(initialReminderItems);
  }, [initialReminderItems, initialized, reminders.length, setReminders]);
    ...
    <div className="-m-6 flex h-[calc(100vh-2.5rem)] flex-col overflow-hidden dark:bg-background">
      <div className="mx-auto flex h-full w-full max-w-6xl flex-col px-6 py-8 md:px-10 md:py-10">
        ...
        <div className="-mr-2 min-h-0 flex-1 space-y-6 overflow-y-auto pr-2 pb-8">
          ...
          <SectionCard title={t('reminders.title')} description={t('reminders.description')}>
