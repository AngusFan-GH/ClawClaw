    ...
    const server = createServer((req, res) => {
      try {
        ...
        if (state !== params.expectedState) {
          ...
          res.end(
            "<!doctype html><html><head><meta charset='utf-8'/></head><body><h2>Session expired</h2><p>This authorization link is from a previous attempt. Please go back to ClawClaw and try again.</p></body></html>"
          );
          return;
        }
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.end(
          "<!doctype html><html><head><meta charset='utf-8'/></head><body><h2>Gemini CLI OAuth complete</h2><p>You can close this window and return to ClawClaw.</p></body></html>"
        );
...
async function discoverProject(accessToken: string): Promise<string> {
  ...
  const headers = {
    ...
    'X-Goog-Api-Client': 'gl-node/clawclaw',
  };
...
export async function loginGeminiCliOAuth(
  ...
  if (ctx.isRemote) {
    throw new Error('Remote/manual Gemini OAuth is not implemented in ClawClaw yet.');
  }
