...
export class GatewayManager extends EventEmitter {
  ...
  private isPathBlocked(toolName: string, params: Record<string, unknown> | undefined): boolean {
    ...
    if (typeof paths === 'string' || (Array.isArray(paths) && paths.every(p => typeof p === 'string'))) {
      return this.doesPathMatchDenied(paths);
    }

    // Write / edit tool: path argument
    const path = params.path ?? params.file_path ?? params.target;
    if (typeof path === 'string' && (toolName === 'write' || toolName === 'edit' || toolName === 'apply_patch')) {
   ...
   * This is used when the desktop host restarts or reloads while OpenClaw
  ...
  private async startProcess(): Promise<import('./config-sync').GatewayLaunchContext> {
    ...
    const useCached = cachedCtx !== null && cachedCtx.port === this.status.port;
