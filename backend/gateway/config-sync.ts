...
import { getManagedPythonEnv, getOpenClawSkillsDir, getOpenClawConfigDir, getOpenClawResolvedDir, getOpenClawDir, getOpenClawEntryPath, isOpenClawPresent, resolveOpenClawDir } from '../utils/paths';
...
async function repairStartupPluginManifests(): Promise<{ repaired: boolean }> {
  ...
  // triggers a re-scan.
  {
    ...
    const result = await runCachedPrelaunchMaintenanceTaskAsync(
 ...
 */
function stableAccountFingerprint(account: Awaited<ReturnType<typeof listProviderAccounts>>[number]): string {
...
export async function prepareGatewayLaunchContext(port: number): Promise<GatewayLaunchContext> {
  ...
  const [
    ...
    withTimeout(
      ...
      { httpProxy: '', httpsProxy: '', allProxy: '', bypassRules: '' },
    ),
  ]);
