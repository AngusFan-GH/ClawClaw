 ...
 * (e.g. during gateway startup) never blocks the desktop backend thread.
...
export function initLogger(): void {
  try {
    ...
    logFilePath = join(logDir, `clawclaw-${timestamp}.log`);

    // Write a separator for new session (sync is OK — happens once at startup)
    const sessionHeader = `\n${'='.repeat(80)}\n[${new Date().toISOString()}] === ClawClaw Session Start (v${app.getVersion()}) ===\n${'='.repeat(80)}\n`;
