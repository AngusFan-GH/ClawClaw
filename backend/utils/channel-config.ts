...
import { resolveOpenClawDir, getOpenClawResolvedDir } from './paths';
...
export async function removeWeChatAccountState(accountId: string): Promise<void> {
    const normalizedAccountId = normalizeOpenClawAccountId(accountId);
    const weChatStateDir = join(resolveOpenClawDir(), WECHAT_RUNTIME_CHANNEL_ID);
    const weChatAccountsDir = join(weChatStateDir, 'accounts');
...
export async function removeQQBotAccountState(accountId?: string): Promise<void> {
