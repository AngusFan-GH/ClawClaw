...
export function getOpenClawCliCommand(): string {
  ...
  if (app.isPackaged) {
    if (platform === 'win32') {
      const cliDir = join(resourcesPath, 'cli');
    ...
    if (platform === 'win32') {
      ...
      return `& ${quoteForPowerShell(process.execPath)} ${quoteForPowerShell(entryPath)}`;
    }
    return `${quoteForPosix(nodeExec)} ${quoteForPosix(entryPath)}`;
  }
...
export function getOpenClawCliSpawnConfig(args: string[]) {
  return { command: process.execPath, args: [getOpenClawEntryPath(), ...args], env: { ...process.env, OPENCLAW_NO_RESPAWN: '1', OPENCLAW_EMBEDDED_IN: 'ClawClaw' }, cwd: getOpenClawDir() };
}
...
function getPackagedCliWrapperPath(): string | null {
  ...
  if (platform === 'darwin' || platform === 'linux') {
    const wrapper = join(resourcesPath, 'cli', 'openclaw');
    return existsSync(wrapper) ? wrapper : null;
  }
  if (platform === 'win32') {
    const wrapper = join(resourcesPath, 'cli', 'openclaw.cmd');
    return existsSync(wrapper) ? wrapper : null;
  }
  return null;
}
...
function getNodeExecForCli(): string {
  if (process.platform === 'win32' && app.isPackaged) {
    const bundledNodePath = join(resourcesPath, 'bin', 'node.exe');
