 ...
 *
 * All file I/O is async (fs/promises) to avoid blocking the desktop backend
 * main thread.
 */
...
import { getResourcesDir, resolveOpenClawDir } from './paths';

const CLAWX_BEGIN = '<!-- clawclaw:begin -->';
const CLAWX_END = '<!-- clawclaw:end -->';
...
/**
 * Merge a ClawClaw context section into an existing file's content.
...
/**
 * Detect and remove bootstrap .md files that contain only ClawClaw markers
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(
            `Removed ClawClaw-only bootstrap file for re-seeding: ${file} (${workspaceDir})`
          );
        } catch {
          logger.warn(`Failed to remove ClawClaw-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge ClawClaw context snippets into workspace bootstrap files that
...
async function mergeClawXContextOnce(): Promise<number> {
  ...
  if (!(await fileExists(contextDir))) {
    logger.debug('ClawClaw context directory not found, skipping context merge');
    return 0;
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.clawclaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.clawclaw.md', '.md');
      ...
      if (merged !== existing) {
        ...
        logger.info(`Merged ClawClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure ClawClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureClawXContext(): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (skipped === 0) {
      logger.info(`ClawClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(
      `ClawClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`
    );
  }

  logger.warn(
    `ClawClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`
  );
}
