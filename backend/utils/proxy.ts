/**
 * Proxy helpers shared by the desktop backend and Gateway launcher.
 */
 ...
 * Desktop host adapters accept scheme-less proxy rules in some cases, but child-process
...
export function buildHostProxyConfig(settings: ProxySettings): HostProxyConfig {
  const mode = settings.proxyMode || (settings.proxyEnabled ? 'custom' : 'system');
