...
function findOpenClawBundledExtension(pluginId: string): string | null {
  const candidateRoots = app.isPackaged
    ? [
        getOpenClawDir(),
        join(resourcesPath, 'app.asar.unpacked', 'node_modules', 'openclaw'),
        join(resourcesPath, 'node_modules', 'openclaw'),
      ]
    : [
        getOpenClawDir(),
...
export function findBundledPluginMirror(pluginId: string): string | null {
  const candidateSources = app.isPackaged
    ? [
        join(resourcesPath, 'openclaw-plugins', pluginId),
        join(resourcesPath, 'app.asar.unpacked', 'build', 'openclaw-plugins', pluginId),
        join(resourcesPath, 'app.asar.unpacked', 'openclaw-plugins', pluginId),
      ]
    : [
        // Dev: bundled plugin mirrors live under the project source tree.
        // The backend bundle is placed under runtime/backend, so relative
        // paths would resolve to a non-existent runtime/build sibling.
