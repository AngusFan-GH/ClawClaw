 ...
 */
import { requestHandlers, HostWindow, shell, dialog, app } from '../host/desktop';
...
function registerHostApiProxyHandlers(): void {
  requestHandlers.handle('hostapi:getBaseUrl', () => {
    return `http://127.0.0.1:${getHostApiPort()}`;
  });

  requestHandlers.handle('hostapi:fetch', async (_, request: HostApiFetchRequest) => {
    ...
    try {
      ...
      const headers: Record<string, string> = { ...(request.headers || {}), Authorization: `Bearer ${hostApiToken}` };
...
function registerUnifiedRequestHandlers(gatewayManager: GatewayManager): void {
  ...
  requestHandlers.handle('app:request', async (_, request: AppRequest): Promise<AppResponse> => {
...
function registerSkillConfigHandlers(): void {
  ...
  // Get skill config
  requestHandlers.handle('skill:getConfig', async (_, skillKey: string) => {
    return await getSkillConfig(skillKey);
  });

  // Get all skill configs
  requestHandlers.handle('skill:getAllConfigs', async () => {
...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  requestHandlers.handle('cron:list', async () => {
    try {
      const result = await gatewayManager.rpc('cron.list', { includeDisabled: true });
      const data = result as { jobs?: GatewayCronJob[] };
  ...
  requestHandlers.handle('cron:update', async (_, id: string, input: Record<string, unknown>) => {
    try {
  ...
  // Delete a cron job
  requestHandlers.handle('cron:delete', async (_, id: string) => {
    try {
  ...
  requestHandlers.handle('cron:toggle', async (_, id: string, enabled: boolean) => {
    try {
  ...
  requestHandlers.handle('cron:trigger', async (_, id: string) => {
    try {
...
function registerUvHandlers(): void {
  ...
  requestHandlers.handle('uv:check', async () => {
    return await checkUvInstalled();
  });

  // Install uv and setup managed Python
  requestHandlers.handle('uv:install-all', async () => {
...
function registerLogHandlers(): void {
  ...
  requestHandlers.handle('log:getRecent', async (_, count?: number) => {
    return logger.getRecentLogs(count);
  });

  // Read log file content (last N lines)
  requestHandlers.handle('log:readFile', async (_, tailLines?: number) => {
    return await logger.readLogFile(tailLines);
  });

  // Get log file path (so user can open in file explorer)
  requestHandlers.handle('log:getFilePath', async () => {
    return logger.getLogFilePath();
  });

  // Get log directory path
  requestHandlers.handle('log:getDir', async () => {
    return logger.getLogDir();
  });

  // List all log files
  requestHandlers.handle('log:listFiles', async () => {
 ...
 */
function registerGatewayHandlers(gatewayManager: GatewayManager, mainWindow: HostWindow): void {
  ...
  // Get Gateway status
  requestHandlers.handle('gateway:status', async () => {
    return await resolveGatewayStatus();
  });

  // Check if Gateway is connected
  requestHandlers.handle('gateway:isConnected', () => {
    return gatewayManager.isConnected();
  });

  // Start Gateway
  requestHandlers.handle('gateway:start', async () => {
    try {
  ...
  // Stop Gateway
  requestHandlers.handle('gateway:stop', async () => {
    try {
  ...
  requestHandlers.handle('gateway:restart', async () => {
    try {
  ...
  requestHandlers.handle('security:updateDeniedPaths', async (_, paths: string[]) => {
    try {
  ...
  requestHandlers.handle('gateway:rpc', async (_, method: string, params?: unknown, timeoutMs?: number) => {
    try {
  ...
  // Health check
  requestHandlers.handle('gateway:health', async () => {
    try {
...
function registerOpenClawHandlers(): void {
  ...
  requestHandlers.handle('openclaw:status', () => {
  ...
  requestHandlers.handle('openclaw:getCliCommand', () => {
    try {
      const status = getOpenClawStatus();
 ...
 */
function registerWhatsAppHandlers(mainWindow: HostWindow): void {
  // Request WhatsApp QR code
  requestHandlers.handle('channel:requestWhatsAppQr', async (_, accountId: string) => {
    try {
  ...
  requestHandlers.handle('channel:cancelWhatsAppQr', async () => {
    try {
  ...
  // requestHandlers.handle('channel:checkWhatsAppStatus', ...)
 ...
 * Device OAuth Handlers (Code Plan)
 */
function registerDeviceOAuthHandlers(mainWindow: HostWindow): void {
  ...
  requestHandlers.handle('provider:cancelOAuth', async () => {
    try {
...
function registerProviderHandlers(gatewayManager: GatewayManager): void {
  ...
  requestHandlers.handle('provider:list', async () => {
  ...
  requestHandlers.handle('provider:listVendors', async () => {
    return await providerService.listVendors();
  });

  requestHandlers.handle('provider:listAccounts', async () => {
    return await providerService.listAccounts();
  });

  requestHandlers.handle('provider:getAccount', async (_, accountId: string) => {
    return await providerService.getAccount(accountId);
  });

  // Get a specific provider
  requestHandlers.handle('provider:get', async (_, providerId: string) => {
    logLegacyProviderChannel('provider:get');
    return await providerService.getLegacyProvider(providerId);
  });

  // Save a provider configuration
  requestHandlers.handle('provider:save', async (_, config: ProviderConfig, apiKey?: string) => {
  ...
  // Delete a provider
  requestHandlers.handle('provider:delete', async (_, providerId: string) => {
  ...
  requestHandlers.handle('provider:setApiKey', async (_, providerId: string, apiKey: string) => {
  ...
  requestHandlers.handle('provider:deleteApiKey', async (_, providerId: string) => {
  ...
  requestHandlers.handle('provider:hasApiKey', async (_, providerId: string) => {
  ...
  requestHandlers.handle('provider:getApiKey', async (_, providerId: string) => {
  ...
  requestHandlers.handle('provider:setDefault', async (_, providerId: string) => {
  ...
  requestHandlers.handle('provider:getDefault', async () => {
...
function registerShellHandlers(): void {
  // Open external URL
  requestHandlers.handle('shell:openExternal', async (_, url: string) => {
    await shell.openExternal(url);
  });

  // Open path in file explorer
  requestHandlers.handle('shell:showItemInFolder', async (_, path: string) => {
    shell.showItemInFolder(path);
  });

  // Open path
  requestHandlers.handle('shell:openPath', async (_, path: string) => {
...
function registerClawHubHandlers(clawHubService: ClawHubService): void {
  // Search skills
  requestHandlers.handle('clawhub:search', async (_, params: ClawHubSearchParams) => {
    try {
  ...
  // Install skill
  requestHandlers.handle('clawhub:install', async (_, params: ClawHubInstallParams) => {
    try {
  ...
  // Uninstall skill
  requestHandlers.handle('clawhub:uninstall', async (_, params: ClawHubUninstallParams) => {
    try {
  ...
  requestHandlers.handle('clawhub:list', async () => {
    try {
  ...
  // Open skill readme
  requestHandlers.handle('clawhub:openSkillReadme', async (_, slug: string) => {
    try {
...
function registerDialogHandlers(): void {
  // Show open dialog
  requestHandlers.handle('dialog:open', async (_, options: Record<string, any>) => {
    const result = await dialog.showOpenDialog(options);
    return result;
  });

  // Show save dialog
  requestHandlers.handle('dialog:save', async (_, options: Record<string, any>) => {
  ...
  // Show message box
  requestHandlers.handle('dialog:message', async (_, options: Record<string, any>) => {
...
function registerAppHandlers(): void {
  // Get app version
  requestHandlers.handle('app:version', () => {
    return app.getVersion();
  });

  // Get app name
  requestHandlers.handle('app:name', () => {
    return app.getName();
  });

  // Get app path overrides for user-facing paths
  requestHandlers.handle('app:getPath', (_, name: Parameters<typeof app.getPath>[0]) => {
  ...
  requestHandlers.handle('app:platform', () => {
    return process.platform;
  });

  // Quit app
  requestHandlers.handle('app:quit', () => {
    quitApp(app);
  });

  // Relaunch app
  requestHandlers.handle('app:relaunch', () => {
    relaunchApp(app);
  });
}
...
function registerSettingsHandlers(gatewayManager: GatewayManager): void {
  ...
  requestHandlers.handle('settings:get', async (_, key: keyof AppSettings) => {
    return await getSetting(key);
  });

  requestHandlers.handle('settings:getAll', async () => {
    return await getAllSettings();
  });

}
function registerUsageHandlers(): void {
  requestHandlers.handle('usage:recentTokenHistory', async (_, limit?: number) => {
    const safeLimit =
      typeof limit === 'number' && Number.isFinite(limit)
        ? Math.max(Math.floor(limit), 1)
        : undefined;
 ...
 * Window control handlers (for custom title bar on Windows/Linux)
 */
function registerWindowHandlers(mainWindow: HostWindow): void {
  requestHandlers.handle('window:minimize', () => {
    mainWindow.minimize();
  });

  requestHandlers.handle('window:maximize', async () => {
  ...
  requestHandlers.handle('window:close', () => {
    mainWindow.close();
  });

  requestHandlers.handle('window:isMaximized', () => {
...
function registerFileHandlers(): void {
  ...
  requestHandlers.handle('file:stage', async (_, filePaths: string[]) => {
    const fsP = await import('fs/promises');
 ...
 *
 * Session lifecycle must go through the Gateway so runtime cleanup and transcript
 * deletion stay aligned with OpenClaw's authoritative session model.
 */
function registerSessionHandlers(gatewayManager: GatewayManager): void {
  requestHandlers.handle('session:delete', async (_, sessionKey: string) => {
    try {
      const normalizedKey = typeof sessionKey === 'string' ? sessionKey.trim() : '';
      if (!normalizedKey) {
        return { success: false, error: 'sessionKey is required' };
      }
      const result = await gatewayManager.rpc('sessions.delete', {
        key: normalizedKey,
        deleteTranscript: true,
      });
      logger.info(`[session:delete] deleted session via gateway: ${normalizedKey}`);
      return { success: true, result };
    } catch (err) {
