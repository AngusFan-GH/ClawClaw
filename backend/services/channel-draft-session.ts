...
export async function beginChannelDraftSession(): Promise<void> {
  ...
  const config = await readOpenClawConfigRecordRaw<OpenClawConfig>().catch((): OpenClawConfig => ({}));
...
export function getChannelDraftConfigSnapshot<T extends OpenClawConfig>(): T | null {
  if (!activeDraftState) {
    return null;
  }
...
export async function commitChannelDraftSession(): Promise<void> {
  ...
  await withConfigLock(async () => {
    const currentConfig = await readOpenClawConfigRecordRaw<OpenClawConfig>().catch((): OpenClawConfig => ({}));
