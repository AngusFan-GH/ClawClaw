...
export async function handleSkillRoutes(
  ...
  if (url.pathname === '/api/clawhub/search' && req.method === 'POST') {
    try {
      ...
      sendJson(res, 200, {
        success: true,
        results: await ctx.clawHubService.search({ ...body, query: String(body.query ?? '') }),
      });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/clawhub/install' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<Record<string, unknown>>(req);
      await ctx.clawHubService.install({ ...body, slug: String(body.slug ?? '') });
  ...
  if (url.pathname === '/api/clawhub/uninstall' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<Record<string, unknown>>(req);
      await ctx.clawHubService.uninstall({ ...body, slug: String(body.slug ?? '') });
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
